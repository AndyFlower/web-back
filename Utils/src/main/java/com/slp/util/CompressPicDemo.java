...
public class CompressPicDemo{  
     ...
     */  
      ...
      public String compressPic(){  
        try{  
            ...
            if(img.getWidth(null) == -1){  
                 ...
                 */  
                tag.getGraphics().drawImage(img, 0, 0, newWidth, newWidth, null);
      ...
      */  
        ...
        try{  
            ...
            if(img.getWidth(null) == -1){  
                ...
                if(this.proportion == true ){  
                    double rate = (double)img.getWidth(null)/(double)outputWidth;  
                    newWidth = outputWidth;  
                    newHeight = ((double)img.getHeight(null)/rate);  
                }else{  
                    newWidth = outputWidth;  
                    newHeight = outputHeight;  
                }                 
      ...
      public String compressPic(String inputDir, String outputDir, String inputFileName, String outputFileName,  
    	  ...
    	  System.out.println(file.isDirectory()+"--"+!file.exists()+outputDir);
    	  if(!file.exists()){
    ...
    public static void main(String[] arg) {  
        ...
        for (int i = 0; i < 7; i++) {//压缩十张图片  
            System.out.println("输入的图片大小：" + mypic.getPicSize("d:/qixbackup/"+i+".jpg") / 1024 + "KB");  
            int start = (int) System.currentTimeMillis(); // 开始时间  
            mypic.compressPic("d:/qixbackup/", "d:/tmp1/", i+".jpg", "r1" + i + ".jpg", 600, 600, true);  
            int end = (int) System.currentTimeMillis(); // 结束时间  
            int re = end - start; // 但图片生成处理时间  
            count += re;  
            System.out.println("第" + (i + 1) + "张图片压缩处理使用了: " + re + "毫秒");  
            System.out.println("输出的图片大小：" + mypic.getPicSize("d:/tmp1/r1" + i + ".jpg") / 1024 + "KB");  
        }  
